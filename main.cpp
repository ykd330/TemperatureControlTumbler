...
void TMDisplayPrint() // 유지 모드 Display 관리 함수
{
  ...
  u8g2.print(userSetTemperature);                                                                      // 설정 온도 출력
  u8g2.drawUTF8((u8g2.getDisplayWidth() - u8g2.getUTF8Width("온도 유지 중")) / 2, 45, "온도 유지 중"); // 설정 온도 출력
  u8g2.setFont(u8g2_font_unifont_h_symbols);                                                           // 폰트 설정
  u8g2.print("℃");                                                                                     // 설정 온도 출력
}
...
void TriggerEnebleFunction()
{
  ...
  PushedButtonTriggerEnableFunction();
  if (TM_count < 0)
  {
    TM_count = 1;
  }
...
void PushedButtonTriggerEnableFunction()
{
...
void PushedButtonTemperatureSettingFunction()
{
  if (upButton == true)
  {
...
void loop()
{
  ...
  case ACTIVE_MODE:
    ...
    {
      PushedButtonTemperatureSettingFunction();
    }

    if (Trigger == true)
    {
      TriggerEnebleFunction();
      if (Trigger_YN == true)
      {
        TriggerYNFunction();
      }
  ...
  case TEMPERATURE_MAINTANENCE_MODE:
    ...
    {
      TMDisplayPrint();
      if (temperatureC + 3 < userSetTemperature)
        changeControlMode(HEATER_MODE);
      else if (temperatureC - 3 > userSetTemperature)
        changeControlMode(COOLER_MODE);
      else
