...
volatile unsigned char activeMode = STOP_MODE; // 온도 설정 모드 초기화

/*---배터리 관리 설정 부---*/
enum BatteryStatus
{
  BATTERY_HIGH = 0, // 배터리 상태 (상)
  BATTERY_LOW = 1   // 배터리 상태 (하)
};
...
// Display 전반을 모두 수정할 예정 -> 한글 폰트의 크기가 너무 큼 -> 적절한 한글 폰트를 찾지 못함 -> 내용을 압축하여 간결하게 출력해야함 --> 완료
// 1. 필요없는 BaseDisplay() 함수 -> batteryDisplay() 함수와 병합 -> 완료
// 4. 그림들을 모두 함수로 직접 그리는 방식으로 변경 -> 완료
...
/*-----ActiveMode Function-----*/
void ActiveModeTriggerEnebleFunction()
{
  u8g2.clearBuffer();
  baseDisplayPrint();
...
void ActiveModeTriggerYNFunction()
{
  if ((TM_count % 2) == 0)
  {
    u8g2.clearBuffer();
...
void loop()
{
  ...
  {
    reBootCheck = millis();
    if (reBootCheck + 5000 < millis())
    {
      esp_sleep_enable_timer_wakeup(5 * 1000000);                        // 5초 후 Deep Sleep 모드로 전환
  ...
  case STANBY_MODE:
    reBootCheck = millis();
    if ((reBootCheck + 300000 < millis()) && digitalRead(BUTTON_BOOT) == LOW)
    {
      gpio_pin_wakeup_enable(BUTTON_BOOT, GPIO_WAKEUP_ENABLE); // GPIO 핀에서 웨이크업 설정
      esp_sleep_enable_gpio_wakeup();                          // GPIO 핀에서 웨이크업 설정
      esp_deep_sleep_start();                                  // 5분간 작동 없으면 Deep Sleep 모드로 전환
      reBootCheck = 0;
    }
    allTumblerDisplayPrint();
    u8g2.sendBuffer();
  ...
  case ACTIVE_MODE:
    ...
    if (Trigger == false && saveMode == false)
    {
      if (upButton == true)
      {
        userSetTemperature++;
    ...
    if (Trigger == true)
    {
      ActiveModeTriggerEnebleFunction();
      if (Trigger_YN == true)
      {
        ActiveModeTriggerYNFunction();
      }
    }
    u8g2.sendBuffer();
    break;
...
void PushedBootButtonLogic()
{
if (bootButton == true && saveMode == false)
  {
